...
class VerboseCodeWriter(type):
    ...
    def __new__(cls, name, bases, attrs):
        attrs = dict(attrs)
        for mname, m in list(attrs.items()):
...
class Node(object):
    ...
    def dump(self, level=0, filter_out=("pos",), cutoff=100, encountered=None):
        ...
        attrs = [(key, value) for key, value in list(self.__dict__.items()) if key not in filter_out]
        if len(attrs) == 0:
...
class DefNode(FuncDefNode):
    ...
    def as_cfunction(self, cfunc=None, scope=None, overridable=True):
        ...
        if cfunc is None:
            ...
            for i, (formal_arg, type_arg) in enumerate(list(zip(self.args, cfunc_type.args))):
                name_declarator, type = formal_arg.analyse(scope, nonempty=1,
...
class ParallelStatNode(StatNode, ParallelNode):
    ...
    def fetch_parallel_exception(self, code):
        ...
        pos_info = chain(*list(zip(self.parallel_pos_info, self.pos_info)))
    ...
    def restore_parallel_exception(self, code):
        ...
        pos_info = chain(*list(zip(self.pos_info, self.parallel_pos_info)))
        code.putln("%s = %s; %s = %s; %s = %s;" % tuple(pos_info))
