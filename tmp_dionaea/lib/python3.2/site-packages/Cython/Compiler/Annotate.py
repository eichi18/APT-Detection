...
class AnnotationCCodeWriter(CCodeWriter):
    ...
    def save_annotation(self, source_filename, target_filename):
        ...
        f.write('<!-- Generated by Cython %s on %s -->\n' % (Version.version, time.asctime()))
        f.write('<html>\n')
        f.write("""
<head>
...
function toggleDiv(id) {
    ...
    if (theDiv.style.display == 'none') theDiv.style.display = 'block';
        ...
        py_marco_api = re.compile('(Py[A-Z][a-z]+_[A-Z][A-Z_]+)\(')
        pyx_c_api = re.compile('(__Pyx_[A-Z][a-z_][A-Za-z_]+)\(')
        pyx_macro_api = re.compile('(__Pyx_[A-Z][A-Z_]+)\(')
        error_goto = re.compile(r'((; *if .*)? \{__pyx_filename = .*goto __pyx_L\w+;\})')
        refnanny = re.compile('(__Pyx_X?(GOT|GIVE)REF|__Pyx_RefNanny[A-Za-z]+)')
        ...
        for line in lines:
            ...
            code, py_c_api_calls = py_c_api.subn(r"<span class='py_c_api'>\1</span>(", code)
            code, pyx_c_api_calls = pyx_c_api.subn(r"<span class='pyx_c_api'>\1</span>(", code)
            code, py_macro_api_calls = py_marco_api.subn(r"<span class='py_macro_api'>\1</span>(", code)
            code, pyx_macro_api_calls = pyx_macro_api.subn(r"<span class='pyx_macro_api'>\1</span>(", code)
            code, refnanny_calls = refnanny.subn(r"<span class='refnanny'>\1</span>", code)
