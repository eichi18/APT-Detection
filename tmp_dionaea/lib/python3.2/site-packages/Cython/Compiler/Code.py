...
cython.declare(re=object, Naming=object, Options=object, StringEncoding=object,
               ...
               DebugFlags=object, none_or_sub=object, str=object)
...
class UtilityCode(object):
    ...
    def specialize(self, pyrex_type=None, **data):
        ...
        key = list(data.items()); key.sort(); key = tuple(key)
        try:
...
class FunctionState(object):
    ...
    def all_free_managed_temps(self):
        ...
        """
        return [(cname, type)
                for (type, manage_ref), freelist in list(self.temps_free.items())
                if manage_ref
...
class GlobalState(object):
    ...
    def generate_string_constants(self):
        c_consts = [ (len(c.cname), c.cname, c)
                     for c in list(self.string_const_index.values()) ]
        c_consts.sort()
        py_strings = []
        ...
        for _, cname, c in c_consts:
            ...
            if c.py_strings is not None:
                for py_string in list(c.py_strings.values()):
    ...
    def generate_int_constants(self):
        consts = [ (len(c.value), c.value, c.is_long, c)
                   for c in list(self.int_const_index.values()) ]
        consts.sort()
        decls_writer = self.parts['decls']
...
class ClosureTempAllocator(object):
    ...
    def reset(self):
        for type, cnames in list(self.temps_allocated.items()):
