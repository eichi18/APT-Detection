...
class PostParse(ScopeTrackingTransform):
    ...
    def visit_LambdaNode(self, node):
        ...
        self.lambda_counter += 1
        node.lambda_name = EncodedString('lambda%d' % lambda_id)
    ...
    def visit_GeneratorExpressionNode(self, node):
        ...
        self.genexpr_counter += 1
        node.genexpr_name = EncodedString('genexpr%d' % genexpr_id)
...
def map_starred_assignment(lhs_targets, starred_assignments, lhs_args, rhs_args):
    ...
    for i, (targets, expr) in enumerate(list(zip(lhs_targets, lhs_args))):
        if expr.is_starred:
            starred = i
    ...
    for i, (targets, expr) in enumerate(list(zip(lhs_targets[-lhs_remaining:],
                                            lhs_args[starred + 1:]))):
        targets.append(expr)
...
class PxdPostParse(CythonTransform, SkipDeclarations):
    ...
    def visit_FuncDefNode(self, node):
        ...
        if isinstance(node, Nodes.CFuncDefNode):
            if 'inline' in node.modifiers and self.scope_type == 'pxd':
...
class InterpretCompilerDirectives(CythonTransform, SkipDeclarations):
    ...
    special_methods.update(list(unop_method_nodes.keys()))
...
#        "threadsavailable",
    ...
    def __init__(self, context, compilation_directive_defaults):
        ...
        for key, value in list(compilation_directive_defaults.items()):
            self.compilation_directive_defaults[str(key)] = copy.deepcopy(value)
    ...
    def visit_ModuleNode(self, node):
        for key, value in list(node.directive_comments.items()):
    ...
    def visit_CImportStatNode(self, node):
        ...
        elif node.module_name.startswith("cython."):
            if node.module_name.startswith("cython.parallel."):
                error(node.pos, node.module_name + " is not a module")
            if node.module_name == "cython.parallel":
                if node.as_name and node.as_name != "cython":
                    ...
                    self.parallel_directives[
                                    "cython.parallel"] = node.module_name
                self.module_scope.use_utility_code(Nodes.init_threads)
            elif node.as_name:
    ...
    def visit_FromCImportStatNode(self, node):
        if (node.module_name == "cython") or \
               node.module_name.startswith("cython."):
            submodule = (node.module_name + ".")[7:]
            newimp = []
    ...
    def visit_FromImportStatNode(self, node):
        if (node.module.module_name.value == "cython") or \
               node.module.module_name.value.startswith("cython."):
            submodule = (node.module.module_name.value + ".")[7:]
            newimp = []
    ...
    def visit_SingleAssignmentNode(self, node):
        if isinstance(node.rhs, ExprNodes.ImportNode):
            ...
            is_parallel = (module_name + ".").startswith("cython.parallel.")

            if module_name != "cython" and not is_parallel:
                return node
    ...
    def visit_CVarDefNode(self, node):
        ...
        for dec in node.decorators:
            for directive in self.try_to_parse_directives(dec.decorator) or ():
                if directive is not None and directive[0] == 'locals':
...
class CreateClosureClasses(CythonTransform):
    ...
    def find_entries_used_in_closures(self, node):
        from_closure = []
        in_closure = []
        for name, entry in list(node.local_scope.entries.items()):
    ...
    def create_class_from_scope(self, node, target_module_scope, inner_node=None):
        ...
        if node.is_generator:
            for entry in list(node.local_scope.entries.values()):
...
class TransformBuiltinMethods(EnvTransform):
    ...
    def visit_cython_attribute(self, node):
        ...
        if attribute:
            ...
            elif attribute in ('set', 'frozenset'):
