...
class DistutilsInfo(object):
    ...
    def merge(self, other):
        if other is None:
            return self
        for key, value in list(other.values.items()):
            type = distutils_settings[key]
...
class DependencyTree(object):
    ...
    def cimported_files(self, filename):
        ...
        b = [_f for _f in [self.find_pxd(m, filename) for m in self.cimports(filename)] if _f]
        if len(a) - int('cython' in a) != len(b):
            print(("missing cimport", filename))
            print(("\n\t".join(a)))
            print(("\n\t".join(b)))
        return tuple(self_pxd + [_f for _f in [self.find_pxd(m, filename) for m in self.cimports(filename)] if _f])
