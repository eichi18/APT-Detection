...
sortkey = lambda name_value: name_value[0].lower()
...
class CyLocals(CythonCommand):
    ...
    def invoke(self, args, from_tty):
        ...
        for name, cyvar in sorted(iter(local_cython_vars.items()), key=sortkey):
...
class CyGlobals(CyLocals):
    ...
    def invoke(self, args, from_tty):
        ...
        for k, v in sorted(iter(global_python_dict.items()), key=sortkey):
            v = v.get_truncated_repr(libpython.MAX_OUTPUT_LEN)
            seen.add(k)
            print('    %-*s = %s' % (max_name_length, k, v))

        print('C globals:')
        for name, cyvar in sorted(iter(module_globals.items()), key=sortkey):
            if name not in seen:
                try:
                    value = gdb.parse_and_eval(cyvar.cname)
                except RuntimeError:
